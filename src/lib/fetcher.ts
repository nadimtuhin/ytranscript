...
import type { Transcript, TranscriptSegment, FetchOptions } from '../types';
...
async function fetchCaptionTrack(
  ...
  try {
    ...
    const data = (await response.json()) as { events?: Array<{ segs?: Array<{ utf8?: string }>; tStartMs?: number; dDurationMs?: number }> };
...
function selectCaptionTrack(
  ...
  for (const lang of preferredLanguages) {
    const match = searchOrder.find(
      (t) => t.languageCode.toLowerCase().startsWith(lang.toLowerCase())
    );
