...
import { errorMessage, formatTranscriptForMcp, textResponse } from './mcp-helpers';
...
server.tool(
  ...
  async ({ video, language = 'en', format = 'text', include_timestamps = false }) => {
    ...
    if (!videoId) {
      return textResponse(`Error: Invalid YouTube video ID or URL: "${video}"`, true);
    }
    ...
    try {
      ...
      return textResponse(formatTranscriptForMcp(transcript, format, include_timestamps));
    } catch (error) {
      return textResponse(`Error fetching transcript for ${videoId}: ${errorMessage(error)}`, true);
    }
  }
);
...
server.tool(
  ...
  async ({ video }) => {
    ...
    try {
      ...
      if (!tracks.length) {
        return textResponse(`No captions available for video ${videoId}`);
      }
      ...
      return textResponse(JSON.stringify({ videoId, languages }, null, 2));
    } catch (error) {
      return textResponse(`Error fetching languages for ${videoId}: ${errorMessage(error)}`, true);
    }
  }
);
...
server.tool(
  ...
  async ({ url }) => {
    ...
    if (!videoId) {
      return textResponse(`Could not extract video ID from: "${url}"`, true);
    }
    return textResponse(videoId);
  }
);
...
server.tool(
  ...
  async ({ videos, language = 'en' }) => {
    ...
    for (const video of videos) {
      const videoId = extractVideoId(video);
      if (!videoId) {
        results.push({ videoId: video, success: false, error: 'Invalid video ID or URL' });
        continue;
      }
      ...
      try {
        ...
        results.push({ videoId, success: true, text: transcript.text });
      } catch (error) {
        results.push({ videoId, success: false, error: errorMessage(error) });
      }
    }
